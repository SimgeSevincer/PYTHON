# -*- coding: utf-8 -*-
"""
Created on Sun Dec 11 14:09:27 2022
...
uyg = QApplication([])
etiket = QLabel("görsel programlama dersi")
etiket.show()
uyg.exec_()
...
etiket = QLabel('< font color ="blue", size = "+3" > görsel programlama dersi </font>' )
dugme = QPushButton('Tıkla') # Bastırılabilen düğme
#pencere.connect(dugme, SIGNAL('pressed()'), metinGuncelle) # Pyqt4 (eski sürüm)
dugme.clicked.connect(metinGuncelle2) # eski sürümde bu satır yerine yukarıdaki satırı deneyiniz.
kutu = QVBoxLayout()
kutu.addWidget(etiket)
kutu.addWidget(dugme)
pencere.setLayout(kutu)
...
class guiUygulama (QDialog):
    def __init__(self, parent=None):
        ...
        kutu.addWidget(dugme)
        self.setLayout(kutu)
        self.setWindowTitle("görsel programlama dersi")
        self.resize(300, 400)
...
class guiUygulama2 (QDialog):
    def __init__(self, parent=None):
        ...
        #Yukarıdaki 3lüyü iptal ederek aşağıdaki izgara 3lüsünü deneyiniz. Görselin anlaşılması beklenmektedir.
        #izgara.addWidget(self.etiket, 0,0,2,1)
        #izgara.addWidget(dugmeKucult,0,1,1,1)
        #izgara.addWidget(dugmeBuyut,1,1,1,1)
